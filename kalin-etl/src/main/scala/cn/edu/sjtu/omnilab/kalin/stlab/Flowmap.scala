...
/**
 * Generate the trip distribution from individual mobility data.
 *
 * Input: (uid, time, loc)
 * Output: (interval, from, to, unique_users, total_users)
 */
object Flowmap extends Serializable{

  /**
   * @param input
   * @param interval the specified time range to aggregate movers.
   * @return
   */
